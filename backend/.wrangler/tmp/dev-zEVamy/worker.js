// .wrangler/tmp/bundle-ipsxTP/checked-fetch.js
...
app.get("/posts", async (c) => {
  ...
  return postBlob.json().then((posts) => Object.entries(posts).map(([slug, post]) => {
    return {
      slug: post.slug,
...
app.get("/posts/:slug", async (c) => {
  ...
  return postBlob.json().then((posts) => posts[slug]).then((post) => {
    c.header("content-type", "application/json");
    c.header("etag", postBlob.httpEtag);
...
app.post(
  ...
  async (c) => {
    ...
    if (action == "update") {
      if (!("message" in body))
        return c.text("Invalid body for update", 400);
      const { success } = await c.env.DB_DRIP.prepare(`
				update drip set message=? where id=?
			`).bind(body["message"], body["id"]).run();
      if (success)
        return c.text(JSON.stringify({ message: "drip updated" }, null, 2), 201);
    } else {
      const { success } = await c.env.DB_DRIP.prepare(`
				insert into drip (message) values (?)
			`).bind(body["message"]).run();
      if (success)
        return c.text(JSON.stringify({ message: "drip created" }, null, 2), 201);
    }
...
app.get("/drip", async (c) => {
  const { success, results } = await c.env.DB_DRIP.prepare(`
	...
	`).bind().all();
  if (!success)
    return c.text(JSON.stringify({ message: "something went wrong" }), 400);
...
app.delete("/drip/:id", async (c) => {
  ...
  const { success } = await c.env.DB_DRIP.prepare(`
	...
	`).bind(id).run();
  if (success)
    return c.text(JSON.stringify({ message: `drip deleted` }), 201);
...
// .wrangler/tmp/bundle-ipsxTP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ipsxTP/middleware-loader.entry.ts
