...
It is easier for an engineer to add a new tool or service to an existing repository where all of their tools have already been configured. Most repos have had configurations added to them to support linters, code formatters, editor
...
is definitely something like having too many repos at the org level.
...
single project repo, but do not work as well with a monorepo. The main decision that doesn't work well is Git tags.
...
There are more technical issues in Git for supporting monorepos, which Atlassian goes into in more depth in their article
"[Monorepos in git](https://www.atlassian.com/git/tutorials/monorepos)".


### Resulting Pain Points

There are some workarounds to get to a point where git can be adapted to host monorepos. However, those adaptations cause
later pain points. 
...
applications coupled through the repository. Loosely coupled applications have been found to be correlated with high rates
...
Dependabot and Renovate are a couple of examples. As of today, neither of these leaders in dependency management supports
the `<app>-<version>` standard of tagging of monorepos. If the software that is in the monorepo is used in other
software (library, SDK, GitHub Actions, Dockerfile, Helm Chart), project-specific tags cannot be used as a source. I
...
find the projects that can easily be removed from the monorepo and moved out to their own. Then find the loosely coupled
...
Stay tuned for the next article where I go in depth on a full streamlined CI/CD flow example of separate projects
