...
migration directory--either manually or with automation if the approach to deployments supports it--and it is run with the
...
in the directory hierarchy above, replacing whatever built-in migration tracking might be used to execute the SQL
...
removed. The next time `edda apply` is run, the finalization migration will be picked up and run in order of creation.
If a new migration is needed that relies on a previous migration being finalized, it needs an ID that is greater than
the _Finalization_ migration (i.e., created after `edda finalize` has been run). This command can also be added to a CI
pipeline to automate migration finalization if the release strategy supports it.

`init`, `create`, `finalize`, and `list` commands only depend on the local file system and having the correct SQL
language for the corresponding relational database. `apply` and `status` interact with the database, so they will require
...
tooling similar to `edda`, and increased regression testing, database changes can happen with the speed we are used to for code changes, along with more stability in database availability. In the next post, we will be exploring the nuances of
