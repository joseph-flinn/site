...
Value Stream. We'll be walking through a full implementation of the flow that can be used as a guideline for new
projects or as a goal for a process migration.


## CD - Continuous Delivery vs Continuous Deployment

CD is a very important part of delivering and sustaining value in today's technological organizations. There are many
organizational benefits. Some of the benefits are: increased product stability, increased velocity in value delivery, which
...
optimize for in the value stream. Those metrics are what have become known as the DevOps metrics: Lead time to
change, change failure rate, deployment frequency, and mean time to recovery. For definitions around these,
...
deployment is the number of manual steps in the pipeline from the time a software engineer commits a change to the
trunk branch until it appears in Production. Here, Production can be defined as the end state where value is handed
...
failure rate of 25%. Let's assume that the organization is able to update processes to release once a week and maintains
the same 25% change failure rate. This means that the number of changes that led to a system failure has increased to 
13. Without digging into the data and what it means, this increase in count can be uncomfortable for executives.
Inserting a manual step right before a deployment/release to Production keeps the decision to release a human one.
...
This is mostly because most application hosting environments are going to be different and the
...
a great resource on what it takes to implement CD and an amazing reference. A key point of understanding that it touches
...
I have seen this important communication tool detract from and get in the way of implementing MinimumCD. We will be walking
through an automatic versioning scheme that fits very well into the developer workflow and will free up any need to worry 
