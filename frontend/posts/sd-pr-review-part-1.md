...
Kanban boards are tools to help visualize work, limit work-in-progress, and maximize...flow (Rehkopf, ?). Ideally, the
...
skills. In addition, this "solution" is only as successful as the availability of the centralized reviewer. As soon as
they take time off, all velocity drops close to zero. 

From a System Dynamics perspective, this is the "Shifting the Burden" archetype (Kim, 1992). Instead of solving the
...
a real solution to the problem. The question is, what is the underlying problem? We could poke and prod the system,
...
Instead, let's use System Dynamics to model the problem, collect data to verify the model and run policy experiments on
