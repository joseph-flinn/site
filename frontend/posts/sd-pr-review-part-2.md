...
working on a model representation of the flow of work that might add delays into the PR review system.
...
been merged. `In Review` is as soon as a PR is created and a review is requested. The first delay in the system is the
...
overlap, intuitively the number of feedback cycles drastically increases the overall PR review completion rate.
...
merging. If the number of feedback cycles is limited, that might lead to an overall cap on the delay for a PR review.
...
in the system that PR reviews are a subsystem of? As I have been learning more about System Dynamics and Causal Loop
Diagrams, I have learned that the above model is not sufficient to understand the complexities of the PR review process
to effectively consider policy changes. More work is needed to better understand the factors at play that are leading to
the delays. Analyzing the possible policy of removing the feedback cycle entirely has shown that the rate of defect
