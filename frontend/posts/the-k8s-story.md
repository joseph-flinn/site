...
needed database changes into the database and run the queries. We would then manually swap the slots
...
points in a system (1999)--to a new and better world.
...
reliable self-healing infrastructure for the service than to scale the current infrastructure
...
issues that we could not track down. .NET has a garbage collector and should automatically free
...
It was my second week as a new manager and I had an idea that was the needed paradigm shift. The
...
that took a step in the right direction) to slowly move towards the vision before others could really
...
Once the team was aligned, we ran into two leadership challenges. There was a change in the role of VP
...
tie to revenue. I have often explained it as we make sure that the value continues to be
available to users while development builds new value. In revenue terms, we make sure that revenue
does not decrease from user churn due to service degradation or outages. We pared down the list of
...
Working through these challenges has been the highlight of my career so far. I am extremely proud
